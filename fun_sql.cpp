...
void list_table(sqlite3 *db) // функция автоматического создания таблицы в базе данных если ее не существует
{
    ...
    else
    {
        ...
        else // запускаем выборку таблиц
        {
            ...
            int num = numTable; //Table; // 

            /* а вот тут мы прогоняем список таблиц заново, но не выводим его а выполняем сравнение и занесение в переменную */
            rc = sqlite3_exec(db, query.c_str(), [num](void *data, int argc, char **argv, char **colName) -> int { // Выполнение SQL-запроса
                if (tableCount == num)                                                                                     // Если номер таблицы совпадает с выбором пользователя
                {
                    //chosenTable = argv[0]; // Запись названия выбранной таблицы в переменную
