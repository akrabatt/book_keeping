 ...
 *
 * @details
 * Внутри функции формируется SQL-запрос для поиска книги по названию, по очереди проходят циклически все наименования таблицы
 * и после того как книга найдена, наименование ее таблицы и самой книги вносится в пару  возвращается
 */
std::pair<std::string, std::string> fing_book(std::vector<std::string> input_vector_tables, std::string book_title)
{
